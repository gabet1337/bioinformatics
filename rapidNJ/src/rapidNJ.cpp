...
polytree* rapidNJ::run(){  
  ...
  mytree->set_serialization_indices(indexToId[index1],indexToId[index2], distance/2.0);
  pb->finish();
...
void rapidNJ::mergeMinNodes(){  
  ...
  // update tree
  /*if(indexToId[obsoleteRowIndex] < 0) {
	  cout << endl << indexToId[obsoleteRowIndex] << " " << obsoleteRowIndex << " " << min2 << " " << redundantToMasterMap[min2]  << " " << clusterCount << endl;
	  cout << min1 << " " << min2 << endl;
	  exit(0);
  }*/
...
int rapidNJ::findRowMin(int i, int rowsize) {
  ...
  for(j = 0; j < rowsize; j++){
    ...
    if(idToIndex[pair.id] != -1) {
	  ...
	  /*if(clusterCount == 421) {
		  cout << i << " " << j << " " << other_cluster_index << " " << global_min << " " << value << " | " << pair.distance << " " << separations[redundantToMasterMap[other_cluster_index]] << " " << row_sep << endl;
	  }*/
      if(value < global_min){
